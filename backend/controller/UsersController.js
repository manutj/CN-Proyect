...
module.exports={
    ...
    update:async(req,res)=>{
        if(req.files){
          ...
          const upload=await utils.upload(profile.tempFilePath);
          if(upload){
    ...
    uploadPhoto:async(req,res)=>{
        ...
        try {
            ...
            if(req.files){
                ...
                const upload=await utils.upload(photo.tempFilePath);
                if(upload){
                    req.body.images=upload.url;
                  }
            UsersService.uploadPhoto(user,req.body.images)
              }
            res.status(200).send('todo bien',user);
          } catch (err) {
            res.status(400).send({ message: 'Error adding image to user', err }); 
          }
    },
    ...
    uploadVideo:async(req,res)=>{
        if(req.files){
          ...
          const upload=await utils.upload(video.tempFilePath);
          if(upload){
              req.body.videos.push(upload.url)
              console.log(upload)
            }
        }
        UsersService.update(req.params.id,req.body)
